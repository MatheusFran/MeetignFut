...
export const AdicionarPessoas = (props) => {
    ...
    const handleImageClick = (value) => {
        if (!selectedImagesList.includes(value)) {
            setSelectedImagesList([...selectedImagesList, value]);
        }
        
    };

    const handleRemoveFromSelectedList = (image) => {
        const updatedList = selectedImagesList.filter((selectedImage) => selectedImage !== image);
        setSelectedImagesList(updatedList);
    };
        ...
        <div className='area-perguntas'>
            <div>
                ...
                <div className="form-group">
                    <label htmlFor="nome">Quem são os outros participantes ?</label>
                    <input
                        type="text"
                    ...
                    <div className="selected-images-list">
                        ...
                        <ul>
                            {selectedImagesList.map((selectedImage) => (
                                <li key={selectedImage}>
                                    {selectedImage}
                                    <button onClick={() => handleRemoveFromSelectedList(selectedImage)}>
                                        <img className="btn-remove" src={remove} alt="Remover" />
                                    </button>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="button-group">
                        <button className="btn" onClick={() => handleRedirect(1)}>
                            Voltar
                        </button>
                        <button className="btn" onClick={() => handleRedirect(2,selectedImagesList)}>
                            Próximo
                        </button>
...
export const useAdicionarPesssoas = () => {
